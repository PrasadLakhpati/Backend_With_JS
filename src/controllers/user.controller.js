...
const loginUser = asyncHandler(async (req, res) => {
    ...
    const isPasswordValid = await user.isPasswordCorrect(password) // Here we are not using User but using user because User only contains built-in methods of mongoose. But user contains manually added methods by User.methods in user.model.js
    if(!isPasswordValid){
    ...
    const options = {// by default cookie can be modified by frontend also. But by applying httpOnly and secure it is only modifiable by server(backend)
        httpOnly: true,
        secure: true
    }
...
const refreshAccessToken = asyncHandler(async (req, res) => { //when users access token expires 401 request comes to user so he has to enter password again to access. to solve this problem google as, when 401 request comes hit one endpoint where refresh your access token(in this request you send your RefreshToken, as soon as backend gets RefreshToken they matches it with the database; if same start session again so new AccessToken is sent and new RefreshToken is stored in database)
    ...
    try {
        ...
        if(incomingRefreshToken !== user?.refreshToken){
            throw new ApiError(401, "Refreh token is expires or used")
        }
...
const updateUserAvatar = asyncHandler( async (req, res) => {
    ...
    if(user.avatar){
        try {
            // Cloudinary stores a public_id for each image.
            // To delete, we need that public_id.
            const oldImageUrl = user.avatar;

            // Extract public_id from the old Cloudinary URL
            // e.g. https://res.cloudinary.com/demo/image/upload/v1234567/foldername/abc123.jpg
